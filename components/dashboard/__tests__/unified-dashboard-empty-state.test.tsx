...
vi.mock("@/components/dashboard/metrics-header-bar", async () => {
  const React = await import("react")
  return { MetricsHeaderBar: () => React.createElement("div", null, "Metrics") }
})

vi.mock("@/components/dashboard/overview-analytics-section", async () => {
  const React = await import("react")
  return { OverviewAnalyticsSection: () => React.createElement("div", null, "Analytics") }
})

vi.mock("@/components/dashboard/insights-panel", async () => {
  const React = await import("react")
  return { InsightsPanel: () => React.createElement("div", null, "Insights") }
})

vi.mock("@/components/dashboard/journal-entries-panel", async () => {
  const React = await import("react")
  return { JournalEntriesPanel: () => React.createElement("div", null, "Journal") }
})

vi.mock("@/components/dashboard/suggestions/kanban-board", async () => {
  const React = await import("react")
  return { KanbanBoard: () => React.createElement("div", null, "Kanban") }
})

vi.mock("@/components/dashboard/calendar", async () => {
  const React = await import("react")
  return { FullCalendarView: () => React.createElement("div", null, "Calendar") }
})
